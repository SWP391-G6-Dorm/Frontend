...
import { TOUCH, extractErr, Spinner, ErrBanner } from './EmployeeShared';
...
export default function RoomInspectionHubPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedId) {
      setFormErr('Vui lòng chọn phòng cần kiểm tra.');
      return;
    }
    ...
    try {
      ...
      setError(extractErr(err, 'Nộp kiểm tra thất bại.'));
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <EmployeeLayout>
      <div style={{ padding: '16px', maxWidth: 520, margin: '0 auto' }} className="animate-fade-in">
        <div style={{ marginBottom: 20 }}>
          ...
          <p className="body-sm text-charcoal">Kiểm tra phòng trước Check-out</p>
        </div>
        ...
        {loadingList ? (
          ...
          <div className="card" style={{ padding: 40, textAlign: 'center' }}>
            ...
            <p className="body-sm text-charcoal">Hiện không có phòng sẵn sàng để kiểm tra.</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="card" style={{ padding: '16px 18px', marginBottom: 14 }}>
              <label className="form-label form-label-required" htmlFor="inspection-pick">
                ...
                <div style={{ width: 80, height: 6, background: 'var(--hairline)', borderRadius: 3, overflow: 'hidden' }}>
                  <div
                    style={{
                      ...
                      height: '100%',
                      background: passCount === totalCount ? '#2b9a66' : 'var(--primary)',
                      borderRadius: 3,
            ...
            <div className="card" style={{ padding: '16px 18px', marginBottom: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                >
                  <input
                    ...
                    style={{ width: 18, height: 18, accentColor: '#2b9a66' }}
                  />
                  <span style={{ fontWeight: 700, color: result === 'PASS' ? '#2b9a66' : 'var(--charcoal)', fontSize: 15 }}>
                    PASS
                  </span>
                </label>
                <label
                ...
                >
                  ...
                  />
                  <span style={{ fontWeight: 700, color: result === 'FAIL' ? '#dc2626' : 'var(--charcoal)', fontSize: 15 }}>
                    FAIL
                  </span>
                </label>
              </div>
              {result === 'FAIL' && (
                <div className="alert alert-error" style={{ marginTop: 12, fontSize: 13 }}>
                  FAIL sẽ chuyển bạn đến tạo báo cáo hư hại.
                </div>
              )}
            </div>
            ...
            <div className="card" style={{ padding: '16px 18px', marginBottom: 16 }}>
              <label
                className={`form-label${result === 'FAIL' ? ' form-label-required' : ''}`}
