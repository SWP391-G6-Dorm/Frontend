...
export default function NotificationCenterPage() {
  ...
  function markAllRead() { setNotifs(p => p.map(n => ({ ...n, isRead: true }))); }
  function markRead(id: string) { setNotifs(p => p.map(n => n.id === id ? { ...n, isRead: true } : n)); }
    ...
    <CustomerLayout>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 24 }}>
          ...
          {unreadCount > 0 && (
            <button onClick={markAllRead} className="btn-ghost btn-sm" style={{ color: 'var(--primary)' }}>Mark all as read</button>
          )}
        </div>
            ...
            {displayed.map((n, i) => (
              <Link key={n.id} to={`/customer/notifications/${n.id}`}
                ...
                style={{
                  ...
                  borderLeft: n.isRead ? 'none' : '3px solid var(--primary)', transition: 'background 0.15s',
                }}
